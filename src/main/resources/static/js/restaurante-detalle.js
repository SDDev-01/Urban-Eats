/* El restaurante y sus platos los renderiza Thymeleaf desde RestauranteDetalleController.
...
/* ---- HERO: el degradado depende del id, por eso se aplica en cliente ---- */
...
function pintarHero() {
  ...
  if (isNaN(id)) return;
  hero.style.background = `linear-gradient(135deg, ${colorPorId(id)} 0%, #1b5e20 100%)`;
}
...
function mostrarVacio(vacio) {
  ...
  aviso.textContent = 'No hay productos disponibles en esta categoría.';
